    ...
    for (const key of Object.keys(soilData) as (keyof SoilData)[]) {
      if (soilData[key] === "" || soilData[key] === null) {
        toast({
          ...
          description: `${t("soil.toast.errorDescription")} ${t(`soil.fields.${key}`)}`,
        });
        return;
      }
    }
        ...
        <Card className="border-2">
          ...
          <CardContent className="grid md:grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              ...
              <FarmerInput
                placeholder={t("soil.fields.soilType_placeholder")}
          ...
          <div className="p-4">
            <FarmerButton variant="primary" size="lg" className="w-full" onClick={handleSave}>
              ...
              <Card key={idx} className="border-2 relative">
                ...
                <CardContent className="grid md:grid-cols-3 gap-4">
                  {["soilType", "ph", "nitrogen", "phosphorus", "potassium", "moisture"].map((key) => (
                    <div key={key} className="space-y-1">
                      <p className="text-sm font-medium">{t(`soil.fields.${key}`)}</p>
                      <p className="text-sm text-muted-foreground">{data[key as keyof SoilData]}</p>
                    </div>
                  ))}
                </CardContent>
              </Card>
